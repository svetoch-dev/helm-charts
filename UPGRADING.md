...
Set `global.envs.<key>.enabled: false` for any environment that must not generate
an Argo CD Application.

Keep `cloud.name` and `short_name` unchanged during this migration if existing
`cloud_short_name` values and resource names must remain unchanged. Changing either
value can make Argo CD create new Applications and delete the old ones.
...
Use wildcard users only when domain-wide access is intended:
...
Remove manually generated `externalEnvs` blocks that duplicate product
environments generated by the chart. Verify datasource UIDs and any templates that
iterate over `externalEnvs` before syncing.
